...
module ApiHelper
  ...
  # from the API.
  def add_deviations_to_db(deviations_array, slideshow_seed = nil, all_uuids = nil)
    ...
    deviations_array.each do |entry|
      ...
      # Check to see if the uuid of the deviation is in the set of known
      # deviations. If it is, set it to true so we can associate it to
      # the slideshow if we have one.
      if all_uuids.include?(uuid)
      ...
      if existing_deviation && slideshow
        ...
        unless slideshow_uuid_set.include?(uuid)
          ...
          begin
            ...
            # insert = "INSERT INTO deviations_slideshows (deviation_id, slideshow_id) VALUES (#{all_uuids[uuid]}, #{slideshow.id})"
  ...
  def get_whats_hot(all_uuids)
    # The set of all deviation UUIDs should be passed in by the MLT worker since it's already collected them.
    # That way it can be handed to the add_deviations_to_db method at the end of this method. Otherwise,
    # the method to retrieve all UUIDs will be needlessly called a second time.
    ...
    while deviations.count <= 250 do
      ...
      add_deviations_to_db(deviations, "00000000-0000-0000-0000-000000000001", all_uuids)
